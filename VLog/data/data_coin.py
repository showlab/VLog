...
class COIN(Dataset):
    def __init__(self, 
        ...
        metadata = os.path.join(dataset_dir, 'coin/metadata', f'{metadata}.json')
        candidate = read_json(metadata)['candidate']
        ...
        self.embed_dir = f'{dataset_dir}/coin/siglip-so400m-patch14-384-fps2'
        # self.embed_dir = f'{dataset_dir}/embed/ego4d_vid16f_lavila_l'
    ...
    def _get_vid_embed(self, sample):
        vid = sample['vid_uid']
        recipe = sample['recipe']
        start, end = sample['segment']
        start = start * self.fps
        end = end * self.fps
        start = math.floor(start)
        end = math.ceil(end)
        
        embed_url = os.path.join(self.embed_dir, str(recipe), f'{vid}.pt')
    ...
    def _get_vid_embed_w_hist(self, sample):
        ...
        for ctx in sample['context']:
            vid_embeds.append(self._get_vid_embed(ctx))
    ...
    def __getitem__(self, idx):
        ...
        output_embeds = self._get_txt(sample)
...
if __name__ == "__main__":
    ...
    # metadata = "coin_next.json"
    metadata = "coin_step+next+task"
    train = True
