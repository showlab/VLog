...
class GenDataset(Dataset):
    ...
    def _get_candidate(self, sample):
        txt_list = [x['clip_text'] for x in sample['candidate']]
        txt_embed = self.vocab_model.get_txt_embeds(txt_list)
        if len(txt_list) == 1:
            txt_embed = txt_embed.unsqueeze(0)
        return txt_embed

    def __getitem__(self, idx):
        sample = self.df[idx]
        
        query_txt = sample.get('question', None)
        response_txt = self._get_txt(sample['answer'])
        ...
        # connected with vocab embed
        # cand_embeds = self._get_candidate(sample)
        # input_embeds = torch.cat([input_embeds, cand_embeds], dim=0)
